/**JavaScript是单线程编程语言，浏览器按照我们代码的顺序一步一步的执行程序
如果执行到一个耗时很长的任务，后面的任务就会阻塞，拖延整个程序的执行。异步编程
技术允许我们执行一个很长的任务时，程序不需要进行等待，而是继续执行后面的代码
直到任务完成之后再回来通知
*/
...
/**
上面的程序会先输出 hello，然后再等待 1 秒后执行 callback 函数，
输出“Hello, JavaScript!”，即使 setTimeout 函数在 console.log("hello") 之前。
*/
...
/**
这种代码极难维护，也被称为 “回调地狱” （callback hell），
现已被 Promise 方案取代。
*/

//Promise
/**
Promise （承诺）是异步编程的现代解决方案，比回调函数方案更强大。
它由社区最早提出和实现，ES6 将其写进语言标准并统一用法，原生提供了 Promise 对象。

Promise 对象有三种状态：
pending（进行中）、fulfilled（已成功）和 rejected（已失败）。
初始状态为 pending，最终状态由异步操作的结果决定。
Promise 对象的状态改变，只有两种可能：
从 pending 变为 fulfilled 和从 pending 变为 rejected。
下面我们用 Promise 重写第一个示例:
*/
// 定义 promise 实例
...
//async函数
/**
我们可以在一个函数前面加上 async 关键字 ，将它变为异步函数，
它的返回值会被自动包装为一个 Promise。
与普通函数不同，异步函数不会阻塞程序的运行，
让 JavaScript 引擎同时处理其他任务：执行其他脚本，处理事件等。
*/
...
//await 关键字
/**
await 关键字只能在 async 函数内工作，
一般情况下，await 后跟随一个 Promise 对象，
作用是让 JavaScript 引擎等待直到 Promise 完成并返回结果。
*/
...
/**下面，我们演示如何使用 async/await 语法
来读取非常流行的无聊猿（BAYC）NFT的元数据。
*/

/**
1、NFT 元数据是构成 NFT 内容的一组数据，通常以 JSON 格式保存在网络上。
比如下面 url 中的 ipfs 链接保存着 id = 1 的BAYC元数据，
包括小图片网址和属性（嘴、头发、衣服等特征）。
*/
...
/**
2、你可以使用 fetch() 函数来进行 HTTP 访问，获取网络数据。
它会返回一个包装成 Promise 的 HTTP 响应，
因此你需要使用 await 关键字来获取结果 response。
然后，你需要用 json() 方法获取 JSON 的内容，也就是元数据。
*/
